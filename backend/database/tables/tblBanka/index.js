...
const getAll = async () => {
  try {
    ...
    const sqlQueries = await utils.loadSqlQueries("tblBanka");
...
const createData = async (createData) => {
  try {
    ...
    const insertData = await pool
      .request()
      .input("bankaadi", sql.VarChar(150), createData.bankaData.bankaadi)
      .input("lokasyon", sql.VarChar(75), createData.bankaData.lokasyon)
